...
float iou(int bbox_1[], int bbox_2[])
{
    ...
    int bbox_1_x = bbox_1[0];
    int bbox_1_y = bbox_1[1];
    int w1 = bbox_1[2] - bbox_1[0];
    int h1 = bbox_1[3] - bbox_1[1];

    int bbox_2_x = bbox_2[0];
    int bbox_2_y = bbox_2[1];
    int w2 = bbox_2[2] - bbox_2[0];
    int h2 = bbox_2[3] - bbox_2[1];

    int endx = std::max(bbox_1_x+w1, bbox_2_x+w2);
    int startx = std::min(bbox_1_x, bbox_2_x);
    int w = w1 + w2 - (endx - startx);

    int endy = std::max(bbox_1_y+h1, bbox_2_y+h2);
    int starty = std::min(bbox_1_y, bbox_2_y);
